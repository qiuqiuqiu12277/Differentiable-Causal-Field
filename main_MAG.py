...
N_sample = 200
meta = pd.read_csv("MAG9.csv")

values = meta.values[:,:-3].astype(float)
names = list(meta.columns[:-3])
...
if __name__ == "__main__":
    ...
    output_path = "./outputs/MAG9"
    os.makedirs(output_path, exist_ok=True)
    
    helper = Helper(datetime_str=datetime_str, N_sample=N_sample, generate=generate, output_path=output_path)
